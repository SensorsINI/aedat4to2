...
def main(argv):
    ...
    if not outputfile.endswith('.aedat'): outputfile=outputfile+'.aedat'
    if outputfile==inputfile:
        log.error('input and output file are the same, would overwrite')
        sys.exit(1)

    print(f'Input file is "{inputfile}" output file will be {outputfile}')
    ...
    log.info(f'loading {inputfile}')
    with AedatFile(inputfile) as f: # TODO load entire file to RAM... not ideal
        ...
        log.info(f'loading events to memory')
        ...
        # log.info(f'converting frames to RAM')
        ...
        # log.info(f'converting IMU samples to RAM')
        # for i in (f['imu']):
        #     a = i.accelerometer
        #     g = i.gyroscope
        #     m = i.magnetometer
...
def export_aedat_2(data, filepath, height=260):
    ...
    with open(filepath, 'wb') as f:
        ...
        output[0::2] = y + x + pol # clear MSB for DVS event https://inivation.github.io/inivation-docs/Software%20user%20guides/AEDAT_file_formats#bit-31
